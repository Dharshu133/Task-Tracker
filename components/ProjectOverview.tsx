  ...
  if (summaries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h3 className="text-white font-semibold text-lg">No projects found</h3>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-slate-900/50 border border-slate-800/50 p-5 rounded-2xl">
      ...
      <div className="space-y-4">
        <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-widest px-1">Project Statistics</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {summaries.map((project) => (
            <button
              key={project.id}
              onClick={() => onProjectSelect(project.id)}
              className="group text-left bg-slate-900 border border-slate-800 rounded-2xl p-6 transition-all duration-300 hover:border-slate-700 hover:shadow-2xl hover:shadow-brand-500/10 hover:-translate-y-1"
            >
              <div className="flex justify-between items-start mb-6">
                <h3 className="text-white font-bold text-lg group-hover:text-brand-400 transition-colors">{project.name}</h3>
                <div className="flex items-center gap-2">
                  {isAdmin && (
                    <button
                      ...
                      className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center hover:bg-red-500/20 text-slate-500 hover:text-red-400 transition-colors"
                  ...
                  <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center group-hover:bg-brand-500/20 transition-colors">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="text-center p-3 rounded-xl bg-slate-950/50 border border-slate-800/50">
                  ...
                  <p className="text-sm font-bold text-white">{project.openTasks}</p>
                </div>
                <div className="text-center p-3 rounded-xl bg-slate-950/50 border border-slate-800/50">
              ...
              <div className="mt-6 pt-6 border-t border-slate-800 flex items-center justify-between">
                ...
                <div className="px-2 py-0.5 rounded-full bg-slate-800 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                  {project.totalTasks}
                </div>
              </div>
            </button>
